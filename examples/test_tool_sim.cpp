...
public:
    ...
    Color get_status_color(TestStatus s)
    {
        ...
        case TestStatus::Running:
            return Theme::current().warning; // Yellow for running
    ...
    void refresh_table()
    {
        ...
        for (const auto &t : tests)
        {
            ...
            {
                if (filter_status == "PASS" && t.status != TestStatus::Passed)
                    continue;
                if (filter_status == "FAIL" && t.status != TestStatus::Failed)
                    continue;
                // simplified filter
            }
    ...
    void update_stats()
    {
        ...
        if (total_progress)
        {
            ...
            if (count_fail > 0)
                total_progress->color = Theme::current().error;
            else if (progress >= 1.0f)
                total_progress->color = Theme::current().success;
            else
                total_progress->color = Theme::current().primary;
        }
    }
