...
public:
    ...
    void tick()
    {
        ...
        for (size_t i = 0; i < cpus.size(); ++i)
        {
            ...
            // Dynamic Color
            if (val < 0.5f)
                cpus[i]->color = Theme::current().success; // Green
            else if (val < 0.8f)
                cpus[i]->color = Theme::current().warning; // Yellow
            else
                cpus[i]->color = Theme::current().error; // Red

            // Update Text
            std::stringstream ss;
            ss << std::fixed << std::setprecision(1) << " " << (val * 100.0f) << "%";
            if (i < percents.size())
                percents[i]->set_text(ss.str());
        }
        ...
        if (spark)
        {
            ...
            if (spark->data.size() > 200)
                spark->data.erase(spark->data.begin());
        }
        ...
        if (list_view)
        {
            for (auto &row : list_view->rows)
            {
                if (row.size() > 6)
                {
                    ...
                    ss_c << std::fixed << std::setprecision(1) << cpu_val;
                    row[5] = ss_c.str();
                    ...
                    ss_m << std::fixed << std::setprecision(1) << mem_val;
                    row[6] = ss_m.str();
                }
            }
        }
...
int main()
{
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        row->add(percent);
        if (i < 4)
            cpu_panel_left->add(row);
        else
            cpu_panel_right->add(row);
    }
    ...
    auto mem_row = std::make_shared<Horizontal>();
    mem_row->height = 1;
    mem_row->fixed_height = 1;
    mem_row->add(make_label("Mem ", 5));
    mem_row->add(mem_bar);
    mem_row->add(make_label(" 4.2G/16G", 10));

    auto swap_row = std::make_shared<Horizontal>();
    swap_row->height = 1;
    swap_row->fixed_height = 1;
    ...
    auto sp2 = make_label(" CPU History:", 15);
    sp2->height = 1;
    sp2->fixed_height = 1;
    stats_panel->add(sp2);
