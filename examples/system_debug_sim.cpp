...
public:
    ...
    void refresh_logs()
    {
        ...
        {
            if (log_level_filter != "ALL" && log.level != log_level_filter)
                continue;
    ...
    void tick()
    {
        ...
        if (cpu_bar)
        {
            ...
            // Color based on load
            if (current_cpu > 0.8f)
                cpu_bar->color = Theme::current().error;
            else if (current_cpu > 0.5f)
                cpu_bar->color = Theme::current().warning;
            else
                cpu_bar->color = Theme::current().success;

            std::stringstream ss;
            ss << (int)(current_cpu * 100) << "%";
            if (cpu_text)
                cpu_text->set_text(ss.str());
        }

        if (mem_bar)
        {
            mem_bar->value = current_mem;
            if (current_mem > 0.85f)
                mem_bar->color = Theme::current().error;
            else
                mem_bar->color = Theme::current().primary;

            std::stringstream ss;
            ss << (int)(current_mem * 100) << "%";
            if (mem_text)
                mem_text->set_text(ss.str());
        }
        ...
        {
            if (current_cpu > 0.8f)
                add_log("WARN", "High CPU load detected!");
            else if (current_mem > 0.9f)
                add_log("ERROR", "Out of memory warning!");
            else
                add_log("INFO", "Routine system check passed.");
        }
    }
};
...
int main()
{
    ...
    // CPU
    auto r1 = std::make_shared<Horizontal>();
    r1->fixed_height = 1;
    ...
    // MEM
    auto r2 = std::make_shared<Horizontal>();
    r2->fixed_height = 1;
    ...
    // DISK
    auto r3 = std::make_shared<Horizontal>();
    r3->fixed_height = 1;
    r3->add(std::make_shared<Label>("DSK: "));
    auto dsk_txt = std::make_shared<Label>("65%");
    dsk_txt->fixed_width = 5;
