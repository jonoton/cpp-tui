...
public:
    void log(const Event& e) {
        ...
        ss << "Type: ";
        switch(e.type) {
            case EventType::None: ss << "None"; break;
            case EventType::Resize:
            ss << "Resize(" << e.x << "x" << e.y << ")";
            break;
        case EventType::Key: 
                ss << "Key(" << (int)e.key << " '" << e.key << "')"; 
                if (e.ctrl) ss << "+Ctrl";
                if (e.shift) ss << "+Shift";
                if (e.alt) ss << "+Alt";
                break;
            case EventType::Mouse: 
                ss << "Mouse(" << e.x << ", " << e.y << ") ";
                if (e.mouse_drag()) ss << "Drag ";
                else if (e.mouse_move()) ss << "Move ";
                else if (e.mouse_left()) ss << "Click(Left) ";
                else if (e.mouse_right()) ss << "Click(Right) ";
                else if (e.mouse_middle()) ss << "Click(Middle) ";
                else if (e.mouse_release()) ss << "Release ";
                else if (e.mouse_wheel()) {
                     if (e.button == 64) ss << "Wheel(Up) ";
                     else if (e.button == 65) ss << "Wheel(Down) ";
                     else ss << "Wheel ";
                }
                else ss << "Unknown(" << e.button << ") ";
            ...
            case EventType::Quit: ss << "Quit"; break;
        }
        
        logs_.push_front(ss.str());
        if (logs_.size() > 50) logs_.pop_back();
    }
    ...
    void render(Buffer& buffer) override {
        ...
        current_y++;
         ...
         // Separator
        for(int i=0; i<width; ++i) buffer.set_char(x+i, current_y, "-");
        current_y++;
