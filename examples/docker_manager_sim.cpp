...
public:
    ...
    void tick()
    {
        ...
        if (tick_count % 5 == 0)
        { // Every 5 ticks update table explicitly
            refresh_table();
        }
        ...
        if (tick_count % 10 == 0)
        {
            ...
            if (!running.empty())
            {
                ...
                if (log_label)
                    log_label->set_text(live_log_line);
            }
        }
    }
    ...
    void action_start()
    {
        ...
        if (notification)
            notification->show("Container " + target_id + " started", Notification::Type::Success);
    }
    void action_stop()
    {
        update_selected_state(ContainerState::Stopped);
        if (notification)
            notification->show("Container " + target_id + " stopped", Notification::Type::Warning);
    }
    void action_restart()
    {
        ...
        update_selected_state(ContainerState::Running);
        if (notification)
            notification->show("Container " + target_id + " restarted", Notification::Type::Info);
    }
        ...
        for (auto &c : containers)
        {
            if (c.id.find(target_id) != std::string::npos && !target_id.empty())
            {
                ...
                if (new_state == ContainerState::Running)
                    c.status_msg = "Up 1 second";
                if (new_state == ContainerState::Stopped)
                    c.status_msg = "Exited (0) just now";
            }
        }
        refresh_table();
    }
    ...
    void refresh_table()
    {
        ...
        for (const auto &c : containers)
        {
            ...
            if (filter_state != "ALL")
            {
                if (filter_state == "RUNNING" && c.state != ContainerState::Running)
                    continue;
                if (filter_state == "STOPPED" && c.state != ContainerState::Stopped)
                    continue;
            }
...
int main()
{
    ...
    auto btn_start = std::make_shared<Button>(" Start ", [manager]()
                                              { manager->action_start(); });
    btn_start->bg_color = Theme::current().success;
    tb_in->add(btn_start);

    auto btn_stop = std::make_shared<Button>(" Stop ", [manager]()
                                             { manager->action_stop(); });
    btn_stop->bg_color = Theme::current().error;
    tb_in->add(btn_stop);

    auto btn_restart = std::make_shared<Button>(" Restart ", [manager]()
                                                { manager->action_restart(); });
